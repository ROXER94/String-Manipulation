...
public class String_Manipulation {
     ...
     *            For s = "abb" and s2 = "bb", return true.
     *            For s = "abbb" and s2 = "bb", return false. 
     */
	public static boolean containsOne(String s, String s2) {
		/**
		 * Do not use a loop or recursion. Instead, look through the methods of
		 * class String and see how you can tell that the first and last
		 * occurrences of s2 in s are the same. Be sure you handle correctly the
		 * case that s2 does not occur in s.
		 */
		return s.indexOf(s2) == s.lastIndexOf(s2) && s.contains(s2);
	}
	...
	public static String fixName(String s) {
		 ...
		 * immutable --you cannot change the sequence of chars that it contains.
		 * 
		 ...
		 * Break the string into its parts 2. Build the result from the parts.
		 */
		s = s.trim();
		int first = s.indexOf(' ');
		int last = s.lastIndexOf(' ');
		return s.substring(last+1, s.length()) + ", " + s.substring(0, first);
    ...
    public static String replaceAllVowels(String s) {
        ...
        //        ...
        //    }
        return s = s.replaceAll("[aeiouAEIOU]", "*");
    }
    ...
    public static String decompress(String s) {
    	...
    	for(int i = 0; i < s.length(); i+=2){
    		String digit = s.substring(i+1,i+2);
